...
{
    public class GenFile_Controller
    {
        public static List<string> AutoGen(dynamic item)
        {
            ...
            string targetPath = System.Web.Hosting.HostingEnvironment.MapPath(@"~/Src/download/Controllers");
            var rs = new List<string>();
            rs.Add(GetGenFile(sourcePath, targetPath, "Controller.cs", item));
            return rs;
        }
        public static string GetGenFile(string sourcePath, string targetPath, string extFileName, dynamic item)
        {
            string fileName = item.menuname+ item.controllername + extFileName;
            // Use Path class to manipulate file and directory paths.
            string sourceFile = Path.Combine(sourcePath, "Create_Controller.cs");
